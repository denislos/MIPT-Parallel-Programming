...
int main(int argc, char **argv)
{
    ...
    const int granulation = (ISIZE - 3) / num_procs;
    const int n_current = ((ISIZE - 3) % num_procs != 0 && rank == num_procs - 1) ? ((ISIZE - 3) - granulation * (num_procs - 1)) : granulation;
    ...
    if (rank == 0)
    {
        ...
        MPI_Recv(&a[granulation * (num_procs - 1)], 
                 ((ISIZE - 3) - granulation * (num_procs - 1)) * JSIZE, 
