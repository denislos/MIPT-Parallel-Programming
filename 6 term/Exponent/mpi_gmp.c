...
//	types[1] = MPI_INT;
	...
	MPI_Get_address(buf + sizeof(int) * 2, &displacements[1]);
	MPI_Get_address(buf + sizeof(int) * 2 + sizeof(mp_exp_t), &displacements[2]);
...
//		printf("%d: (%d, %d) %d\n", i, types[i], blockcounts[i], displacements[i]);
	...
	MPI_Type_create_struct(3, blockcounts, displacements, types, mpi_mpf_t);
